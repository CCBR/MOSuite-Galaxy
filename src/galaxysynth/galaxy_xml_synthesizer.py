...
class GalaxyXMLSynthesizer:
    ...
    def _add_command(self, tool: ET.Element, tool_id: str):
        ...
        for param in self.blueprint.get("parameters", []):
            ...
            if param_type == "BOOLEAN":
                ...
                param_type == "LIST" or param_type == "VECTOR" or param_type == "ARRAY"
            ):
    ...
    def _add_inputs(self, tool: ET.Element):
        ...
        if not ordered_keys:
            ...
            ordered_keys = []
            ordered_keys.extend(
                [d["key"] for d in self.blueprint.get("inputDatasets", [])]
            )
            ordered_keys.extend(
                [p["key"] for p in self.blueprint.get("parameters", [])]
            )
        ...
        sorted_sections = sorted(
            sections.items(), key=lambda x: section_first_index.get(x[0], float("inf"))
        )
    ...
    def _add_parameter(self, parent: ET.Element, param_def: Dict[str, Any]):
        ...
        if param_type == "LIST" or param_type == "VECTOR" or param_type == "ARRAY":
        ...
        elif param_type == "SELECT" or param_type == "MULTISELECT":
            # Handle SELECT and MULTISELECT type parameters
            param = ET.SubElement(parent, "param")
            param.set("name", param_key)
            ...
            if param_values:
                for value in param_values:
                    ...
                    # Only set selected for SELECT (not MULTISELECT)
                    if (
                        param_type == "SELECT"
                        and default_value
                        and str(value) == str(default_value)
                    ):
    ...
    def _generate_help(self) -> str:
        ...
        if has_special_params:
            help_lines.extend(["", "**Special Characters Allowed:**", ""])

            # Document Anchor_Neighbor_List specifically
            if "Anchor_Neighbor_List" in [
                p.get("key") for p in self.blueprint.get("parameters", [])
            ]:
                help_lines.extend(
                    [
    ...
    def _format_xml(self, elem: ET.Element) -> str:
        ...
        result = re.sub(
            r"<!\[CDATA\[(.*?)\]\]>", unescape_cdata, result, flags=re.DOTALL
        )
...
def batch_process(input_pattern: str, output_dir: str, docker_image: str = "spac:mvp"):
    ...
    generated_files = []
    feature_summary = {"sanitizer": [], "sections": [], "integer": [], "float": []}
